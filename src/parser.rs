...
pub enum AstNode {
    ...
    FunctionDeclaration { id: String, args: Vec<String>, body: Box<AstNode> }, 
    ...
    ArrayCall { id: String, index: Option<Box<AstNode>> },
...
impl ProgramAST {
    fn parse_keyword(&mut self) -> AstNode {
        ...
        match self.at() {
            ...
            Identifier(id) if &id[..] == "fn" => {
                ...
                let id = if let AstNode::Identifier(name) = self.parse_keyword() { name }
                ...
                let args: Vec<String> = if self.at() != &(Paren{open: false}) {
                    let args = self.parse_csvs();
                    if self.at() != &(Paren{open: false}) { error("Expected ClosingParen"); }
                    args.iter().map(|e| if let AstNode::Identifier(name) = e {
                        name.to_string()
                    } else {
                ...
                return AstNode::FunctionDeclaration { id, args, body: Box::new(body) };
            },
    ...
    fn parse_block(&mut self) -> AstNode {
        ...
        while self.at() != &(Brace {open: false}) {
            ...
            body.push(node);
            if self.at() == &(Brace {open: false}) { break; }
    ...
    fn parse_assignment(&mut self) -> AstNode {
        let mut left = self.parse_condition_chain();
        ...
        if let AstNode::Identifier(ref name) = left {
            ...
            self.check_next("Expected value after Equals");
            let right = self.parse_condition_chain();
    ...
    fn parse_object(&mut self) -> AstNode {
        ...
        if self.at() != &(Brace {open: true}) { return self.parse_additive(); }
    ...
    fn parse_member(&mut self) -> AstNode {
        ...
        if self.at() != &Identifier("".to_string()) || !self.has_next() { return self.parse_primary(); }
        let parent = self.parse_primary();
            
        if self.next() != &Dot { self.last(); self.parse_primary(); }
        self.expect(Identifier(String::new()), "Expected member identifier after Dot");

        return AstNode::MemberCall{ parent: Box::new(parent), member: Box::new(self.parse_primary()) };
    }
    ...
    fn parse_primary(&mut self) -> AstNode {
        ...
        match self.at().clone() {
            ...
            Identifier(name) => {
                ...
                if self.next() != &(Bracket{open: true}) { self.last(); return id; }
                self.check_next("Expected index or ClosingBracket"); // throws error if no next
                if self.at() == &(Bracket{open: false}) { 
                    return AstNode::ArrayCall { id: name.clone(), index: None }; }

                let index = self.parse_assignment();
                // throws error if next isnt the specifier token
                self.expect(Bracket{open: false}, "Expected ClosingBracket"); 

                return AstNode::ArrayCall { id: name.clone(), index: Some(Box::new(index)) };
            },
